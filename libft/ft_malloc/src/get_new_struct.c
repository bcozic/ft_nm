...
static t_mem	*get_new_data_page(void)
{
	...
	if (g_data->allocate_size + g_data->pages_size
		...
		return (0);
	map = mmap(NULL, g_data->pages_size * 10, PROT_READ | PROT_WRITE,
			MAP_ANON | MAP_PRIVATE, -1, 0);
	g_data->allocate_size += g_data->pages_size * 3;
	new_page = (t_mem*)map;
	new_page->ptr = map;
	new_page->size = g_data->pages_size * 3;
	new_page = insert_new_element(&(&g_data->data_page)->page, new_page);
	...
	new_packet = insert_new_element(&(&g_data->data_page)->packet, new_packet);
	return (new_packet);
}
