...
/*   Updated: 2018/12/15 20:49:56 by bcozic           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_ppc_little_32(uint32_t *ptr, uint32_t addr, t_data *data, uint32_t size)
{
	...
	while (i < size)
	{
		...
		ft_printf("%08zx\t%08x %08x %08x %08x \n", addr + i, ptr[0], ptr[1], ptr[2], ptr[3]);
		i += 16;
		ptr = ptr + 4;
	}
}
...
void	print_ppc_little_64(uint32_t *ptr, uint64_t addr, t_data *data, uint64_t size)
{
	...
	while (i < size)
	{
		...
		ft_printf("%016zx\t%08x %08x %08x %08x \n", addr + (uint32_t)i, ptr[0], ptr[1], ptr[2], ptr[3]);
		i += 16;
		ptr = ptr + 4;
	}
}
...
void	print_ppc_big_32(uint32_t *ptr, uint32_t addr, t_data *data, uint32_t size)
{
	...
	while (i < size)
	{
		...
		ft_printf("%08zx\t%08x %08x %08x %08x \n", addr + i, lte_32(ptr[0]), lte_32(ptr[1]), lte_32(ptr[2]), lte_32(ptr[3]));
		i += 16;
		ptr = ptr + 4;
	}
}
...
void	print_ppc_big_64(uint32_t *ptr, uint64_t addr, t_data *data, uint64_t size)
{
	...
	while (i < size)
	{
		...
		ft_printf("%016zx\t%08x %08x %08x %08x \n", addr + (uint32_t)i, lte_32(ptr[0]), lte_32(ptr[1]), lte_32(ptr[2]), lte_32(ptr[3]));
		i += 16;
		ptr = ptr + 4;
	}
}
